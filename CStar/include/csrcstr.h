	...
	namespace util
	{
		namespace cstr
		{
			...
			inline void copy(char*& dest, const char* source) noexcept
			{
				if (!dest)
				{
					std::size_t len = length(source) + 1;
					dest = new char[len];
			...
			inline void copy(char*& dest, const char* source, const std::size_t maxCnt) noexcept
			{
				if (!dest)
				{
					std::size_t len = length(source, maxCnt) + 1;
					dest = new char[len];
					for (std::size_t i = 0; i < len; i++)
						dest[i] = source[i];
					dest[len - 1] = '\0';
			...
			{
				std::size_t strL = length(str);
				std::size_t sufL = length(suf);
			...
			template<typename IntType, typename std::enable_if<std::is_integral<IntType>::value, bool>::type = 0>
			inline IntType toInt(const char* str) noexcept
			{
				IntType mul = 1;
				IntType result = 0;
				std::size_t len = length(str) - 1;
				for (;; len--)
				{
			...
			inline unsigned int split(const char* str, char**& subStrs, char delim) noexcept
			{
				...
				for (std::size_t i = 0; c < cnt; i++)
				{
					if (str[i] == delim)
					{
						...
						{
							subStrs[c] = nullptr;
							copy(subStrs[c++], str + lastIndex, i - lastIndex + 1);
							wasDelim = true;
						}
						lastIndex = i + 1;
					}
			...
			/// Helper class for distance (see below)
			class cstrdist_H
			{
			private:
				...
				explicit cstrdist_H(const char* _str1, const char* _str2, unsigned int** _vals, bool** _vms, const unsigned int _str1l, const unsigned int _str2l)
					: str1{ _str1 }, str2{ _str2 }, vals{ _vals }, vms{ _vms }, str1l{ _str1l }, str2l{ _str2l } {}
				friend inline unsigned int distance(const char* str1, const char* str2) noexcept;
			public:
			...
			/// Examples:
			///		distance("abcde","abcXde") => 1
			///		distance("abcde","abXde") => 1
			///		distance("abcde","abde") => 1
			///		distance("abcde","abdce") => 2
			///		distance("aaaaa","bbb") => 5
			///		distance("aaaaa","bbbbbb") => 6
			inline unsigned int distance(const char* str1, const char* str2) noexcept
			{
				unsigned int l1 = (unsigned int)length(str1) + 1;
				unsigned int l2 = (unsigned int)length(str2) + 1;
				...
				unsigned int result = cstrdist_H(str1, str2, vals, vms, l1 - 1, l2 - 1)();
				for (unsigned int k = 0; k < l1; k++)
				{
					delete[] vals[k];
					delete[] vms[k];
			...
			inline char** alignL(const char* strs[], const std::size_t& numStrs, const std::size_t& targetLen) noexcept
			{
				std::size_t* lens = new std::size_t[numStrs];
				std::size_t maxLen = 0;
				for (std::size_t i = 0; i < numStrs; i++)
				{
					lens[i] = length(strs[i]);
			...
			/// targetlen : fills strings to be of length targetLen
			/// Note:	targetLen will be ignored if it is too small.
			inline char** alignR(const char* strs[], const std::size_t& numStrs, const std::size_t& targetLen) noexcept
			{
				std::size_t* lens = new std::size_t[numStrs];
				std::size_t maxLen = 0;
				for (std::size_t i = 0; i < numStrs; i++)
			...
			/// targetlen : fills strings to be of length targetLen
			/// Note:	targetLen will be ignored if it is too small.
			inline char** alignC(const char* strs[], const std::size_t& numStrs, const std::size_t& targetLen) noexcept
			{
				std::size_t* lens = new std::size_t[numStrs];
				std::size_t maxLen = 0;
				for (std::size_t i = 0; i < numStrs; i++)
...
namespace cstar
{
	namespace cstr
	{
		using csr::util::cstr::length;
		using csr::util::cstr::length;
		using csr::util::cstr::copy;
		using csr::util::cstr::copy;
		using csr::util::cstr::equal;
		using csr::util::cstr::prefix;
		using csr::util::cstr::suffix;
		using csr::util::cstr::toInt;
		using csr::util::cstr::split;
		using csr::util::cstr::distance;
		using csr::util::cstr::alignL;
		using csr::util::cstr::alignR;
		using csr::util::cstr::alignC;
		using csr::util::cstr::blocktext;
	}
}
